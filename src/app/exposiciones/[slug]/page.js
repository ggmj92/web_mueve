...
export async function generateMetadata({ params }) {
  ...
  const artworks = (exposicion.artworks || []).filter(
    (a) => a?.image?.asset?.url
  )
  ...
  const descText =
    exposicion.description
      ...
      .join(' ')
      .slice(0, 160) || `${exposicion.title} - Exposición en Mueve Galería`
  ...
  const artistNames =
    exposicion.artists
      ...
      .filter(Boolean)
      .join(', ') || ''
  const titleWithArtists = artistNames
    ? `${exposicion.title} - ${artistNames}`
    : exposicion.title
      ...
      <main>
        ...
        {artworks.length > 0 && (
          <section className={styles.cardsSection}>
            <ExposicionCarousel artworks={artworks} exposicionSlug={slug} />
          </section>
        )}
      </main>
    </>
