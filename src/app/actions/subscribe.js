...
export async function subscribe(formData) {
  ...
  if (MAILERLITE_API_KEY && MAILERLITE_GROUP_ID) {
    try {
      ...
      return {
        ok: false,
        message: 'Could not subscribe right now. Please try later.',
      }
    } catch (error) {
  ...
  if (ENABLE_LOCAL_EMAIL_STORAGE === 'true') {
    ...
    return {
      ok: false,
      message: 'Could not save your email. Please try later.',
    }
  }
  ...
  return {
    ok: false,
    message: 'Newsletter service not configured. Please contact us directly.',
  }
}
