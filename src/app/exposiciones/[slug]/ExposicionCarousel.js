    ...
    return (
        <div className={styles.carouselContainer}>
            ...
            {displayArtworks.map((aw, index) => {
                ...
                return (
                    <div key={`${aw.image.asset._id}-${index}`} className={styles.cardWrapper}>
                        <div className={styles.card}>
                            <picture>
                                <source media="(max-width: 768px)" srcSet={mobileImageUrl} />
                                <img
                                    src={desktopImageUrl}
                                    alt={aw.artworkInfo?.[0]?.title || 'Exposición image'}
                                    className={styles.cardImage}
                                />
                            </picture>
                        </div>
                        {aw.artworkInfo && aw.artworkInfo.length > 0 && (
                            <div className={styles.cardInfo}>
                                {aw.artworkInfo.map((info, infoIndex) => (
