    ...
    <div className={styles.carouselContainer}>
      ...
      >
        {displayArtworks.map((aw, index) => {
          ...
          const imageSlug =
            aw.slug?.current || `${exposicionSlug}-imagen${index + 1}`
                ...
                {aw.artworkInfo?.map((info, infoIdx) => (
                  <div key={infoIdx} className={styles.cardInfoBlock}>
                    ...
                    <div className={`${styles.cardTitle} notranslate`}>
                      {info.title}
                      {info.year && (
                        <span className={styles.cardYear}>, {info.year}</span>
                      )}
                    </div>
                    {info.technique && (
                      <div className={styles.cardDetail}>{info.technique}</div>
                    )}
                    {info.dimensions && (
                      <div className={styles.cardDetail}>{info.dimensions}</div>
                    )}
                  </div>
                ))}
              </div>
